...
app.post("/newuserpc", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      message: "User and project creator request added successfully",
    });
  } catch (error) {
    console.error(
      "Error creating new user and project creator request:",
      error
    );
    return res.status(500).json("server_error");
  }
...
app.post("/application/delete", async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      "DELETE FROM post_request WHERE request_id = $1",
      [e]
    );
...
app.post("/ProjectCreator/newproject", async (req, res) => {
  try {
    const { project_title, category, date, img, user_id } = req.body;
    await client.query(
      "INSERT INTO project (project_title, category, date, img, user_id) VALUES ($1, $2, $3, $4, $5)",
      [project_title, category, date, img, user_id]
    );
...
app.post("/ProjectCreator/updateproject", async (req, res) => {
  try {
    const { project_title, category, date, img, project_id } = req.body;
    console.log(project_title, category, date, img, project_id);
    await client.query(
      "UPDATE project SET project_title = $1, category = $2, date = $3, img = $4 WHERE project_id = $5",
      [project_title, category, date, img, project_id]
    );
...
app.post("/ProjectCreator/getmanager", async (req, res) => {
  try {
    ...
    const manager = await client.query(
      `SELECT project_manager.*, users.f_name, users.l_name, users.username, users.role
...
app.post("/ProjectCreator/addManager", async (req, res) => {
  try {
    ...
    await client.query(
      "UPDATE users SET role = 'ProjectManager' WHERE user_id = $1",
      [user_id]
    );
...
app.post("/ProjectCreator/newpost", async (req, res) => {
  try {
    ...
    await client.query(
      "INSERT INTO post (project_id, post_details) VALUES ($1, $2)",
      [project_id, detail]
    );
...
app.post("/ProjectManager/updateStatus", async (req, res) => {
  try {
    ...
    const postdata = await client.query(
      `UPDATE post_request SET status = $1 WHERE request_id = $2`,
      [status, request_id]
    );
