...
const Requests = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post('http://localhost:3000/admin/getAllRequestDataWithUser');
  ...
  const accept_projectcreater = async (user) => {
    try {
      await axios.post('http://localhost:3000/admin/acceptProjectCreator', { user });
      fetchData();
    } catch (error) {
  ...
  const reject_projectcreater = async (user) => {
    try {
      await axios.post('http://localhost:3000/admin/rejectProjectCreator', { user });
      fetchData();
    } catch (error) {
            ...
            <div className="Request-main-content">
              <div className="containter">
                <div className="Request-content">
                  ...
                  <h3>{request.f_name} {request.l_name}</h3>
